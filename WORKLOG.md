...
## 2026-09-16 — Claude Code (2차)

### FC다움 매장 목록 자체 중복 제거 + 중복 매장 후보 찾기 도구 (배포 완료)

- 1차 수정(아래 항목, `583f868`) 배포 후 사용자가 실제 확인해보니 매장 폼 관리에 매장명이 여전히 두 개씩 남아있음을 확인 — 1차 수정은 "앞으로" 새 중복만 막는 비파괴적 방식이라 배포 이전 기존 중복은 그대로 남아있는 게 원인.
- 재조사로 추가 구멍 2개 발견: (1) `fetchAllStores()`(`fcdaum.ts`)가 QSC 리포트 조회와 달리 storeNo 기준 중복 제거가 없어 FC다움 응답 자체의 중복을 그대로 통과시킴 → `Set` 기반 dedup 추가. (2) storeNo 비교가 `String()` 그대로라 엑셀 텍스트 서식의 앞자리 0 등에 취약 → `normalizeStoreNo()`로 숫자 정규화.
- 기존 중복(배포 전 생성분)은 자동 병합 대상이 아니라 관리자가 찾아서 수동 숨김 처리해야 하는데 ~600개 규모에서 눈으로 찾기 어려워, "중복 매장 후보 찾기" 도구(`DuplicateStoreFinder.tsx`) 신설 — storeNo 동일/이름·주소 유사 그룹을 나열하고 기존 숨김 기능(`storeHidden.ts`)과 바로 연결. 관리 → 데이터 관리 탭에 배치.
- 사용자가 로컬(Firestore 직접 읽기라 FC다움 API 없이도 테스트 가능)에서 실행해 **86건**의 중복 후보를 확인함 — 배포 후 실제 정리는 사용자가 도구로 직접 진행.

---

## 2026-09-16 — Claude Code

### FC다움 재동기화 시 매장 중복 생성 + 시도명 불일치 수정 (배포 완료)

- **원인**: 엑셀 임포트(`StoreImportPanel.tsx` 문서ID=관리번호)와 FC다움 API 동기화(`fcdaum.ts` 문서ID=storeId)가 같은 매장을 서로 다른 문서ID로 저장 — 이전에 엑셀로 등록된 매장을 FC다움으로 재동기화하면 `existingMap.get(p.id)`가 못 찾아 `state:'new'`로 잡혀 중복 문서 생성(WORKLOG bf2f2fa에서 이미 진단, "재키잉 필요·리스크 있어 보류"였던 사안).
- **수정(비파괴적, 재키잉 없음)**: FC다움 `storeNo`(=엑셀 관리번호와 동일)를 두 경로 공통 매칭키로 사용. 기존 문서에 `storeNo` 필드가 없으면 문서 id가 숫자일 때 그 id 자체를 fallback으로 사용해 즉시 매칭(`buildStoreNoMap`). 매칭되면 새 문서 대신 기존 문서에 병합 저장(`targetId`).
- 강원/전북/제주 시도명(신·구) 정규화 유틸 `src/storeRegion.ts` 신설, "변경됨" 판정에 `region` 포함해 재동기화 시 기존 문서 지역명도 자동 교정. `StoreMindmapView.tsx` 지역 그룹핑에도 방어적으로 적용.
- 기존에 이미 생성된 중복 매장은 자동 병합 대상 아님 — 가맹 관리(`StoreMgmtView.tsx`) 화면의 기존 "숨김 처리" 기능으로 수동 정리(사용자 선택). 로컬은 `/api/fcdaum` 서버리스 라우트가 없어 "불러오기" 테스트 불가 — 배포 후 사용자 확인 예정.
...
- **체크리스트 부서탭 배지 오류**: `OpenChecklistView`의 탭 배지가 이미 부서필터링된 목록을 재필터링해 탭 누를 때마다 숫자가 흔들리던 버그 — 전체목록(`unifiedListAll`)/표시목록(`unifiedList`) 분리로 수정.
- **도면업로드 연동 누락**: `systemAction`(도면 등) 태그가 옛 마이그레이션 완료 브랜드엔 없어 체크리스트 업로드해도 배너가 안 사라지던 문제 — 3차 마이그레이션으로 자동 백필.
- **FC다움 QSC 회귀(실서버 검증)**: `fetchQscReportsAll`이 page/pageSize 무시를 놓쳐 브랜드 최신 10건만 받고 종료 → 상위 10위 밖 매장은 점검해도 영구 미확인. `to`(방문일 상한) 기반 시간역순 페이징으로 전량(123건) 수집 확인. 캐시 v5→v6.

### 회의록 이월/공지 연동 + 내 업무 공간 개선 (배포 완료, firestore.rules는 사용자가 콘솔 게시)

- 회의록: 안건·결정·실행항목 이월 시 `[이월]` 텍스트 제거, 실행항목 이월 시 원본 자동완료(트랜잭션, 업무보고 중복집계 방지), 결정사항→공지사항 전송 버튼.
- 업무보고: 회의 실행항목 패널에 "내 것만" 필터(최근 5개 제한은 유지).
- 내 업무 공간: 로그인 기본화면으로 지정, 일정 없어도 미니캘린더 상시표시(기존 데이터 재활용), 메모·담당업무와 별개인 "할 일" 체크리스트(`personal_todos`) 신설.
...
### R&D 관리대장 완성·배포 — 현재 안정 상태

- **위치**: 사이드바 '보고' 그룹 → 제조실 다음 'R&D 관리대장'(section `rnd`, `src/components/RndView.tsx`). 배포됨(saemoyangfms, 번들 RndView-CswfBOT5).
- **최종 구조(2탭)**: **품목**(목록↔상세: 8단계 공정 체크시트+항목별 메모/이슈+자동 진행보고, 현재 단계 전체 체크 시 다음 단계 자동 진행, 완료 제조품→제조실 이관) / **캘린더**(품목 시작·목표일+계획일정 자동 표기, R&D 전용). 일일/주간/월별 수기작성 없음 — 보고 전부 자동.
- **데이터**: salesDb `rnd_items`만 사용(checklist/schedule 포함). rnd_daily·rnd_weekly·rnd_monthly는 폐지(구 문서 방치·무해). **firestore.rules는 콘솔 수동 게시**(CLI 403).
- **판단 보류**: 한 달 운영 후 실사용(체크·메모 축적) 보고 존속 결정 — 안 쓰이면 메뉴만 내리면 됨.
- 미착수(이전): 가맹관리 첫 화면 현황 박스 확대(2026-06-29 기록).
...
- 인쇄 영역(#rnd-print-area)에 `tab==='calendar'` 분기 추가 — 월 그리드를 표로 출력(▶시작 ★목표완료 ·계획). 기존엔 캘린더에서 인쇄 시 품목 목록 표가 나왔음. 빌드 ✓.
...
- **배경**: 사용자가 "구조 복잡·중복 많음. 딱 이 정도가 좋다" — ①각 체크리스트 ②각 체크마다 메모(이슈) ③항목별 진행/남은공정 자동 보고 ④월 계획 날짜→캘린더 자동. 캘린더는 R&D 전용, 날짜별 전체 일지 뷰는 불필요 확정.
- **제거**: 일일기록(rnd_daily)·주간보고 수동작성(rnd_weekly)·월별표(rnd_monthly) UI 전부 삭제. 로드도 rnd_items만. (기존 문서는 방치 — 무해)
- **체크시트**: 항목마다 메모/이슈(MessageSquare 아이콘, blur 자동저장), doneAt 기록. 현재 단계 전체 체크 시 다음 단계 자동 진행(기존 유지).
- **자동 보고**: 상세 우측 '진행 보고' = 진행사항/남은 공정(단계별)/이슈사항 전부 체크 상태에서 자동 생성. 카톡·인쇄도 자동. 수기 작성 0.
- **R&D 전용 캘린더**: 품목 startDate(시작)·targetDate(목표완료)·schedule[](계획일정) 자동 표기. 월 그리드, 이벤트 클릭→품목 상세. 상세에 '계획 일정' 추가/삭제 UI.
- 타입: RndItem.checklist에 doneAt/memo/memoAt, schedule[] 추가. 빌드 ✓.

## 2026-07-03 — Claude Code (3차)

### R&D 2탭 통합 (품목/보고서) + 체크 시 단계 자동 진행

- **2탭 재편**: 일일/주간/월별 탭 폐지 → **품목**(목록↔상세: 체크시트+일일기록+'금주 보고' 이슈·차주계획·지원요청 입력까지 전부) / **보고서**(주간·월간 전환: 주간=일일기록 자동 집계, 월간=계획+주차 실적). "쓰는 곳 하나, 보는 곳 하나".
- **단계 자동 진행**: 체크박스 체크로 현재 단계 항목이 전부 완료되면 다음 미완료 단계로 자동 이동(빈 단계는 건너뜀, 8단계 전체 완료 시 완료 처리·제조실 이관 안내 토스트). 체크 해제로는 강등 안 함(수동 select).
- 상세 일일기록에 이번 주/전체 필터. 금주 보고 코멘트는 상세·보고서 양쪽에서 수정 가능(같은 `rnd_weekly` 문서).
- handleSaveWeeklyExtra에 periodStart 파라미터 추가 — 상세는 항상 실제 이번 주, 보고서는 선택한 주 기준. 빌드 ✓.

## 2026-07-03 — Claude Code (2차)

### R&D 통합 구조 개편 — 체크시트 상시 노출 + 일일/주간/월별 연동

- **품목 상세 화면 신설**: 관리대장에서 품목명 클릭 → 좌 공정 체크시트(모달 아님, 상시 펼침, 체크 즉시 저장) + 우 일일기록 타임라인·빠른 입력(Ctrl+Enter). 단계/상태/제조실 이관/수정 모두 상세 헤더에서.
- **체크시트 항목 편집**: '항목 편집' 토글로 품목별 항목 추가/삭제. 데이터 `rnd_items.checklist[{stage,text,done}]` (구 stageChecks 자동 마이그레이션). ChecklistModal 제거.
- **주간보고 자동 생성**: 별도 작성 폐지 — 주 선택(◀▶)하면 해당 주 일일기록을 품목별 자동 집계. 이슈/차주계획/지원요청만 blur 자동저장 보완 입력(`rnd_weekly` id=`${itemId}_${주 월요일}`). WeeklyModal 제거, progressNote는 optional(구 수동작성분 표시용).
- **월별계획 연동**: 품목명 일치 시 W1~W5 셀에 해당 주차 일일기록 수 자동 뱃지.
- **인쇄**: 상세 화면 인쇄=공정 체크시트 리포트(☑/☐+기록 전체), 주간 인쇄=자동 집계본. 빌드 ✓.

## 2026-07-03 — Claude Code

### R&D 단계를 실제 8단계 공정으로 교체 + 공정 체크시트

- **배경**: 사용자가 실제 공정 제공 — ①표준레시피 ②시범생산·가치파악 ③원가검토 ④대량생산검증 ⑤포장개발 ⑥매장테스트 ⑦문제점·개선 ⑧차주계획(출시준비). 기존 임의 6단계를 이 8단계로 교체(진행률 10/25/40/55/70/85/92/100%).
- **공정 체크시트**: 단계별 세부 항목(맛확인·관능평가·라벨제작 등, RND_STAGES.subs) 체크박스 모달. 행의 체크시트 아이콘 또는 단계 셀 '체크 n/n' 클릭. 단계 제목 클릭=현재 단계 설정. `rnd_items.stageChecks: Record<'단계-idx', boolean>`.
- ⚠️ 기존 품목의 stage 번호(1~6)는 새 8단계 라벨로 재해석됨 — 품목 적을 때 교체라 수동 재지정 안내.
- 빌드 ✓.
...
### 오픈일정 체크리스트 인쇄 빈 페이지 수정

- **원인**: index.css 전역 `@media print { body * { visibility: hidden } }`(캘린더 인쇄 도입 시 추가)가 화이트리스트 id 없는 화면을 전부 숨김 → OpenChecklistView는 `print:` 클래스만 쓰고 전용 id가 없어 빈 페이지 출력(회귀).
- **수정**: 상세 패널에 `id="checklist-print-area"` 부여 + index.css 화이트리스트 블록 추가(다른 인쇄 영역과 동일 패턴). 빌드 ✓.
- ⚠️ 같은 방식(전용 id 없이 window.print)인 화면이 더 있을 수 있음 — 인쇄 안 되는 화면 발견 시 동일 패턴으로 수정.
...
### R&D 후속: 제조품/일반상품 구분 + 상태 필터 + 제조실 이관 (✅ 완료·배포됨)

- **✅ 종결(2026-07-02)**: 커밋 `e7ee314`(신규 메뉴) + `b9096d3`(구분/필터/이관) 모두 push → saemoyangfms.vercel.app 배포 확인(번들 RndView-BAQYN2Sj). **firestore.rules는 사용자가 콘솔에서 수동 게시**해 저장 정상화. 규칙 CLI 배포는 403 지속(계정 권한) — 다음에도 규칙 변경 시 콘솔 수동 게시 필요.
- **배경**: 저장 실패(Missing permissions) → firestore.rules에 rnd_* 4개 컬렉션 규칙 추가. ⚠️ **CLI 배포 403**(wnsdl9331 계정에 firebaserules 권한 없음, reauth로도 동일) → **Firebase 콘솔에서 규칙 수동 반영 필요**.
- **구분 추가**: `productType: 제조품(제조실 직접 제조) | 일반상품(업체 직거래 납품)`. 관리대장 기본 필터=진행중, 상태 칩(진행중/보류/완료/중단/전체)으로 아카이브 분리 — 완료·보류·중단은 이동 없이 필터 뒤로.
- **제조실 이관**: 완료된 **제조품만** 행에 공장 아이콘 → 단위/안전재고 입력 모달 → `factory_items` 생성(이름 중복 방지) + `factoryItemId` 연결(재이관 방지, 등록됨 표시). 일반상품은 이관 없음.
- 순서 이동은 필터 뷰 내 이웃과 order 스왑 방식으로 변경. 빌드 ✓.
