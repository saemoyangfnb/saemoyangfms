...
import { Search, X, MapPin, Phone, User as UserIcon, Calendar, ChevronRight, Building2, Clock } from 'lucide-react';
...
export function StoreListView({ currentUser }: Props) {
  ...
  const filtered = useMemo(() => stores.filter(s => {
    ...
    if (search) {
      ...
      return s.name?.toLowerCase().includes(q) || s.region?.toLowerCase().includes(q) || s.ceoName?.toLowerCase().includes(q) || s.storeCode?.toLowerCase().includes(q);
    }
    return true;
  }), [stores, search, filterStatus]);

  // 매장의 오픈 이력: storeId 매칭 우선, 없으면 storeName 매칭
  ...
  return (
    <div className="flex gap-0 h-[calc(100vh-8rem)] -mx-4 sm:-mx-6 lg:-mx-8">
      ...
      {/* ── 좌: 매장 목록 ── */}
      <div className={`flex flex-col shrink-0 border-r border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 transition-all ${selected ? 'w-80' : 'w-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-stone-100 dark:divide-stone-800">
          {filtered.length === 0 ? (
            ...
            filtered.map(store => (
              <button
                key={store.id}
                onClick={() => setSelected(prev => prev?.id === store.id ? null : store)}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="divide-y divide-stone-100 dark:divide-stone-800">
                {[
                  ...
                  <div key={row.label} className="flex items-center gap-3 px-4 py-2.5">
                    {row.icon && <span className="text-stone-400 shrink-0">{row.icon}</span>}
            ...
            <div className="bg-white dark:bg-stone-900 rounded-sm border border-stone-200 dark:border-stone-700 overflow-hidden">
              <div className="px-4 py-3 border-b border-stone-100 dark:border-stone-800 flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-stone-400">{storeSchedules.length}건</span>
              </div>
              {storeSchedules.length === 0 ? (
                <div className="py-8 text-center">
                  <p className="text-xs font-bold text-stone-300 dark:text-stone-600">연결된 오픈 일정이 없습니다</p>
                  <p className="text-[10px] text-stone-300 dark:text-stone-700 mt-1">franchise_schedules에서 이 매장의 storeId가 매핑되어야 합니다</p>
                </div>
              ) : (
                <div className="divide-y divide-stone-100 dark:divide-stone-800">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="text-xs font-black text-stone-800 dark:text-stone-200">
                            {sch.archived ? '오픈완료' : '진행중'}
                          </span>
                          {sch.archived && <span className="text-[9px] font-bold text-stone-400 bg-stone-100 dark:bg-stone-800 px-1.5 py-0.5 rounded-sm">보관됨</span>}
                        </div>
                        <div className="flex items-center gap-3 text-[11px] font-bold text-stone-500">
                          {sch.openDate && <span>오픈일 {sch.openDate}</span>}
                          {sch.team && <span>· {sch.team}</span>}
                        </div>
                        {sch.constructionStart && (
                          <p className="text-[10px] text-stone-400 mt-0.5">공사 {sch.constructionStart}{sch.constructionEnd ? ` ~ ${sch.constructionEnd}` : ''}</p>
                        )}
                      </div>
                    </div>
                  ))}
