...
import {
  ...
  ShieldAlert, TrendingUp, TrendingDown, BarChart2,
...
type TabType = 'overview' | 'store' | 'marketing' | 'competitor';
...
function RankBadge({ rank }: { rank: number }) {
  const r = Number(rank);
  if (r >= 999) return (
    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 border border-slate-200 dark:border-slate-700">
      노출 실패
    </span>
  );
  if (r <= 3) return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border border-amber-100 dark:border-amber-800">
      <Trophy size={9} /> {r}위
    </span>
  );
  if (r <= 5) return (
    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800">
      {r}위
    </span>
  );
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-rose-50 dark:bg-rose-900/20 text-rose-700 dark:text-rose-400 border border-rose-100 dark:border-rose-800">
      {r}위
    </span>
  );
}
                  ...
                  <div className="mt-3 space-y-3">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => onResolve(review.id)}
                        className="flex-1 py-2 text-xs font-semibold bg-slate-900 dark:bg-blue-600 text-white rounded-lg hover:bg-slate-700 dark:hover:bg-blue-700 transition-colors">
                        해피콜 조치 완료
                      </button>
                      <button onClick={() => onOverride(review.id)}
                        className="flex-1 py-2 text-xs font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors border border-slate-200 dark:border-slate-700">
                        긍정 예외 처리
                      </button>
                    </div>
                  </div>
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {stores.length === 0 ? (
                ...
                <div key={item.store} className="px-5 py-3 flex items-center gap-3">
                  ...
                  <span className={`text-sm font-bold shrink-0 ${item.count > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}`}>
                    {item.count}건
                  </span>
                </div>
              ))}
...
function StoreTab({ reviews }: { reviews: Review[] }) {
  ...
  const filteredStores = searchQuery
    ? allStores.filter(s => s.replace(/ /g, '').includes(searchQuery.replace(/ /g, '')))
    : allStores;

  useEffect(() => {
    if (filteredStores.length > 0 && !filteredStores.includes(selectedStore)) {
...
function MarketingTab({ rankData, roiData }: { rankData: RankData[]; roiData: RoiData[] }) {
  ...
  const latestDate = useMemo(() => {
    const dates = rankData.map(r => r.수집일자).sort();
  ...
  const sortedRoi = useMemo(() =>
    [...roiData].sort((a, b) => parseRate(b.키워드_적중률) - parseRate(a.키워드_적중률)),
    [roiData]
  );
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-200 dark:border-slate-800">
          ...
          <div className="flex gap-3">
            ...
            <select value={selectedStore} onChange={e => setSelectedStore(e.target.value)}
              className="flex-1 py-2 px-3 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 dark:focus:ring-blue-500 text-slate-900 dark:text-white">
              {filteredStores.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {sortedRoi.map(roi => {
                ...
                return (
                  <tr key={roi.id}
                    className={`hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors cursor-pointer ${selectedStore === roi.매장명 ? 'bg-blue-50 dark:bg-blue-900/10' : ''}`}
                    onClick={() => setSelectedStore(roi.매장명)}>
                    <td className="px-4 py-3 text-sm font-medium text-slate-800 dark:text-slate-200">{roi.매장명}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${rate >= 70 ? 'bg-emerald-400' : rate >= 40 ? 'bg-amber-400' : 'bg-rose-400'}`}
                    ...
                    <td className="px-4 py-3">
                      {isNoKeyword
                        ? <span className="text-xs text-amber-600 dark:text-amber-400 font-medium">미설정</span>
...
function CompetitorTab({ competitorData }: { competitorData: CompetitorData[] }) {
  ...
  const latestDate = useMemo(() => {
    const dates = [...new Set(competitorData.map(c => c.수집일자))].sort();
    return dates[dates.length - 1] || '';
  }, [competitorData]);

  const prevDate = useMemo(() => {
    const dates = [...new Set(competitorData.map(c => c.수집일자))].sort();
    return dates.length >= 2 ? dates[dates.length - 2] : '';
  }, [competitorData]);
  ...
  function getPriceRange(brand: string, data: CompetitorData[]) {
    const prices = data.filter(d => d.경쟁브랜드명_엑셀.includes(brand))
      .map(d => getMackerelPrice(d.메뉴_및_가격)).filter(p => p > 0);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {TARGET_BRANDS.map(brand => {
          ...
          if (latestRange.min > 0 && prevRange.min > 0) {
            ...
            if (diff > 0) trendEl = (
              <span className="inline-flex items-center gap-1 text-xs font-semibold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/20 px-2 py-0.5 rounded-full border border-rose-100 dark:border-rose-800">
                <ArrowUp size={9} /> {diff.toLocaleString()}원 인상 감지
              </span>
            );
            else if (diff < 0) trendEl = (
              <span className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded-full border border-blue-100 dark:border-blue-800">
                <ArrowDown size={9} /> {Math.abs(diff).toLocaleString()}원 인하 감지
              </span>
            );
              ...
              {brandData.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                  {brandData.slice(0, 3).map((store, idx) => {
                    const menuItems = store.메뉴_및_가격 && store.메뉴_및_가격 !== '수집 실패'
                      ? store.메뉴_및_가격.split('|').slice(0, 5) : [];
                    return (
                      <div key={idx} className="px-5 py-3">
                        ...
                        {store.타겟_키워드 && store.타겟_키워드 !== '미설정' && (
                          <p className="text-xs text-slate-400 mt-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                            타겟 키워드: <span className="text-blue-600 dark:text-blue-400 font-medium">{store.타겟_키워드}</span>
                          </p>
                        )}
                      </div>
                    );
...
export function ReviewDashboard() {
  ...
  const handleResolve = async (id: string) => {
    ...
    try {
      await setDoc(doc(db, 'review_states', 'resolved'), { ids: newIds, updated_at: new Date().toISOString() });
    } catch { }
  };
